...
void main() {
  ...
  // test('deleteSpecificFile throws Other Exception(Jwt is null)', () async {
  //   final jwtService = MockJwtService();
  //   final httpClient = MockClient();
  //
  //   const String testId="Test id";
  ...
  //   when(httpClient.delete(url, headers: {'Authorization': 'Bearer ${jwtService.getJwt()}'})).thenAnswer(
  //         (_) async => http.Response('', 555),
  //   );
  //   final FilesService apiService = FilesService(jwtService: jwtService, httpClient: httpClient);
  //
  //   await apiService.deleteSpecificFile(testId);
  //   expect(apiService.deleteSpecificFile(testId), throwsA(isA<Exception>()));
  //
  // });
